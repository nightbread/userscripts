...
const toM = (input: string) =>
  input
    .replace(
      /^h([0-6])\.(.*)$/gm,
      (_, level, content) =>
        `${Array(parseInt(level, 10) + 1).join('#')}${content as string}`,
    )
    .replace(
      /([*_])(.*)\1/g,
      (_, wrapper, content) =>
...
const toJ = (input: string) =>
  input
    .replace(
      /^(.*?)\n([=-])+$/gm,
      (_, content, level) =>
        `h${(level as string[])[0] === '=' ? 1 : 2}. ${content as string}`,
    )
    .replace(
      /^([#]+)(.*?)$/gm,
      (_, level, content) =>
        `h${(level as any[]).length}.${content as string}`,
    )
    .replace(
      /([*_]+)(.*?)\1/g,
    ...
    .replace(
      ...
      (_, from: keyof typeof MAP, content: string) =>
        `${MAP[from]}${content}${MAP[from]}`,
    )
    .replace(/~~(.*?)~~/g, '-$1-')
    .replace(
      /`{3,}(\w+)?((?:\n|[^`])+)`{3,}/g,
